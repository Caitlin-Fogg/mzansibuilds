...
def create_comment(project_id: int, comment: schemas.CommentCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),):
    ...
    return schemas.CommentResponse(id=db_comment.id, content=db_comment.content, user_id=db_comment.user_id, username=current_user.username, project_id=db_comment.project_id, created_at=db_comment.created_at)
...
def get_comments(project_id: int, db: Session = Depends(get_db)):
    ...
    comments = (db.query(models.Comment, models.User.username).join(models.User, models.Comment.user_id == models.User.id).filter(models.Comment.project_id == project_id).all())

    # Unpack tuples and return response
    return [schemas.CommentResponse(id=comment.id, content=comment.content, user_id=comment.user_id, username=username, project_id=comment.project_id, created_at=comment.created_at)
...
def update_comment(comment_id: int, updated: schemas.CommentCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user),):
    ...
    return schemas.CommentResponse(id=comment.id, content=comment.content, user_id=comment.user_id, username=current_user.username, project_id=comment.project_id, created_at=comment.created_at)
